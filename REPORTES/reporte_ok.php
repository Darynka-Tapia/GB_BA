<?php
	...
	//Consulta
	$sql = "select 	
	(select COUNT(p.Folio1) FROM prestamo_respaldo p where p.Folio1>0) as CantLibros1, 
    (select COUNT(p.Folio2) FROM prestamo_respaldo p where p.Folio2>0 ) as CantLibros2, 	
    (SELECT COUNT(L.Folio) from libro L) AS CantidaLibros, 
    (SELECT COUNT(v.Tipo_vista) FROM visitas v where v.Tipo_vista='Individual') as PrestamosInternos";
	$resultado = $mysqli->query($sql);
	$fila = 7; //Establecemos en que fila inciara a imprimir los datos
	
	$gdImage = imagecreatefrompng('images/logoBachi.jpg');//Logotipo
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B3', 'REPORTE DE LIBROS');
	$objPHPExcel->getActiveSheet()->mergeCells('B3:D3');
	
	$objPHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(10);
	$objPHPExcel->getActiveSheet()->setCellValue('A6', 'FOLIO');
	$objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(30);
	$objPHPExcel->getActiveSheet()->setCellValue('B6', 'TITULO');
	$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(30);
	$objPHPExcel->getActiveSheet()->setCellValue('C6', 'AUTOR');
	$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(30);
	$objPHPExcel->getActiveSheet()->setCellValue('D6', 'FORMA DE INGRESO');
	$objPHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(30);
	$objPHPExcel->getActiveSheet()->setCellValue('E6', 'ID ASIGNATURA');
	...
	while($rows = $resultado->fetch_assoc()){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $Domicilio);
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$fila, $row['PrestamosInternos']);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$fila, $row['CantidaLibros']);
	...
	$values = new PHPExcel_Chart_DataSeriesValues('Number', 'Libros!$D$7:$D$'.$fila);
	
	// definir origen de los rotulos
	$categories = new PHPExcel_Chart_DataSeriesValues('String', 'Libros!$B$7:$B$'.$fila);
